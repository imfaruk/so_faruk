...
RSpec.describe API::Resources::Categories do
  include Rack::Test::Methods

  describe 'GET /categories' do
    let(:request_url) { '/api/v1/categories' }
    ...
    it 'matches with expected_response' do
      VCR.use_cassette('categories') do
        response = get request_url
  ...
  describe 'GET /categories/:name/recipes' do
    let(:request_url) { "/api/v1/categories/#{category_name}/recipes" }
    let(:category_name) { 'Beef' }
    let(:expected_response) do
      [
        {
          'id' => '52874',
    ...
    it 'returns a response of application/json type' do
      VCR.use_cassette('category_recipes') do
        response = get request_url
        expect(response.content_type).to eq('application/json')
      end
    end
